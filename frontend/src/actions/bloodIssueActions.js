...
export const listBloodIssue = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5000/api/blood-issue`)
...
export const createBloodIssue = (blood) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`http://localhost:5000/api/blood-issue`, blood, config)
...
export const deleteBloodIssue = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`http://localhost:5000/api/blood-issue/${id}`, config)
