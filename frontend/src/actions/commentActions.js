...
export const listComment = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5000/api/comment`, config)
...
export const createComment = (comment) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`http://localhost:5000/api/comment`, { comment }, config)
...
export const updateComment = (comment) => async (dispatch, getState) => {
  try {
    ...
    await axios.put(`http://localhost:5000/api/comment/${comment._id}`, comment, config)
...
export const deleteComment = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`http://localhost:5000/api/comment/${id}`, config)
